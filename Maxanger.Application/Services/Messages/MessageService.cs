    ...
    : IMessageService
{
    public async Task<IMessages> SendMessageAsync(IMessageOnSend messageOnSend)
    {
        ...
        var response = await mediator.Send(new SendMessageCommand(
            userId.Value,
            messageOnSend.ChatId,
            encryptor.Encrypt(messageOnSend.Content),
            messageOnSend.MessageType,
            messageOnSend.Metadata,
            messageOnSend.ReplyToId));
            ...
            if (replyMessage != null)
            {
                ...
                [
                    new SentMessage
                    {
                        ...
                        Content = encryptor.Decrypt(replyMessage.Content),
